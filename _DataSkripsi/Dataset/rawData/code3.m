...
% Biru  = [2201 3200, 5401 6400, 8601 9600]
...
% -------------------------------------------------------- -----Output: dataPB dataPM-------

% % ----------------------------------- BandPass M B ---------------Input: dataNBp-------
% for j=chAwal:chAkhir
%     dataBpM(:,j)=filter(d,c,dataNBp(:,j)); % dataBpM = BP 10.5 - 11.5 Hz
% end
% for j=chAwal:chAkhir
%     dataBpB(:,j)=filter(f,e,dataNBp(:,j)); % dataBpB = BP 12.5 - 13.5 Hz
% end
% -------------------------------------------------------- -----Output: dataBpM dataBpB-------

% % ----------------------------------- Potong Data ---------------Input: dataBpM dataBpB-------
% for j=chAwal:chAkhir
%     dataPM(:,j) = dataBpM(601:length(dataBpM),j); % potong data 3 detik awal (3*200 = 600) - Fs=200Hz
% end
% for j=chAwal:chAkhir
%     dataPB(:,j) = dataBpB(601:length(dataBpM),j); % potong data 3 detik awal (3*200 = 600) - Fs=200Hz
% end
% tP = [0:length(dataPM)-1]/fs;
